...
from insurance_autodml._crossfit import cross_fit_nuisance
from insurance_autodml._inference import eif_inference
from insurance_autodml._types import DoseResponseResult, OutcomeFamily
from insurance_autodml.riesz import ForestRiesz
...
class DoseResponseCurve:
    ...
    def plot(
        ...
        if show_data_rug:
            ax.plot(
                self._D,
                np.full_like(self._D, result.ate.min() - 0.02 * (result.ate.max() - result.ate.min())),
                "|",
                color="grey",
                alpha=0.3,
                markersize=3,
